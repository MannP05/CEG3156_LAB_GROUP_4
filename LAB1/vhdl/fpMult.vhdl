...
ARCHITECTURE structural OF fpMult IS
    ...
    COMPONENT controlUnit_Mult
        PORT (
            clk, reset, multiplier_bit          : IN  STD_LOGIC;
    ...
    COMPONENT bigALU
        ...
        PORT (
            i_Mantissa_A, i_Mantissa_B : IN  STD_LOGIC_VECTOR(BITS-1 DOWNTO 0);
    ...
    COMPONENT mux_2x1
        GENERIC (BITS : INTEGER);
        PORT (
            in_0, in_1 : IN  STD_LOGIC_VECTOR(BITS-1 DOWNTO 0);
            in_sel     : IN  STD_LOGIC;
    ...
    COMPONENT bidirectional_shifter
        ...
        PORT (
            ...
            i_enable, i_direction  : IN  STD_LOGIC;
...
BEGIN
    ...
    PORT MAP (
        clk            => GClock,
        reset          => GReset,
        multiplier_bit => Reg_B_Out(0),
        load_inputs    => ctrl_load,
        shift_enable   => ctrl_shift,
        add_enable     => ctrl_add,
        done_flag      => ctrl_done
    );
    ...
    U_REG_A : nbit_register
    GENERIC MAP (18)
    PORT MAP (
        in_val      => Reg_A_In,
        in_load     => '1',
        in_resetBar => s_resetBar,
        in_clock    => GClock,
        out_val     => Reg_A_Out
    );

    U_SHIFT_A : bidirectional_shifter
    GENERIC MAP (18)
    PORT MAP (
        i_val       => Reg_A_Out,
        i_enable    => ctrl_shift,
    ...
    U_SHIFT_B : bidirectional_shifter
    GENERIC MAP (9)
    PORT MAP (
        i_val       => Reg_B_Out,
        i_enable    => ctrl_shift,
    ...
    GENERIC MAP (18)
    PORT MAP (
        i_Mantissa_A => Prod_Out,
        i_Mantissa_B => Reg_A_Out,
        i_Op_Code    => '0',
        o_Result     => ALU_Add_Out,
        o_CarryOut   => OPEN,
        o_Overflow   => OPEN,
        o_Zero       => OPEN
    );
    ...
    PORT MAP (
        ...
        in_1    => ALU_Add_Out,
        in_sel  => ctrl_add,
        out_mux => Mux_Prod_Calc
    );

    U_MUX_LOAD_PROD : mux_2x1
    GENERIC MAP (18)
    PORT MAP (
        in_0    => Mux_Prod_Calc,
        in_1    => Zero_Vec_18,
    ...
    GENERIC MAP (8)
    PORT MAP (
        i_Mantissa_A => ExpBiased,
        i_Mantissa_B => Norm_Bit_Vec,
        i_Op_Code    => '0',
        o_Result     => ExpNorm,
        o_CarryOut   => OPEN,
        o_Overflow   => OPEN,
